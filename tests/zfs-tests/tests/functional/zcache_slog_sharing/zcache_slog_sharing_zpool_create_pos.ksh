...
#	device and partition 2 is the cache. The log device can be added during
#	the pool creation
#
# STRATEGY:
...
log_assert "Verify that the shared device(s) are partitioned into two parts." \
	...
	" device. The slog devices are specified during the pool creation"
...
log_pass "Partitioning the shared device and using it as a slog device" \
	" succeeded with zpool create operation"
