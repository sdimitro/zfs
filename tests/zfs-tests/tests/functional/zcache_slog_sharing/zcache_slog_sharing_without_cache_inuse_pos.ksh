...
#	device and partition 2 is the cache. The log device can be added
#	to a pool irrespective of zettacache being used or not
#
# STRATEGY:
...
#	   zcache device
#	3. Create a pool and add all of the slog devices
#	4. Verify that the pool was created
#	5. Verify that the log devices are online for the pool
#
...
log_assert "Verify that the shared device(s) are partitioned into two parts." \
	...
	" device. We should be able to just use the slog devices configuring the cache" \
	" devices"
