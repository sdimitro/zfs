...
#	device and partition 2 is the cache. The pool can be exported and imported
#	back with the zpool import command
#
# STRATEGY:
...
#	8. Import the pool and verify that the import was successfull
#	7. Verify that the log devices are still online
#
...
log_assert "Verify that the shared device(s) are partitioned into two parts." \
	...
	" device. The slog devices are specified during the pool creation and the pool" \
	" when exported is available for import with the slog devices inplace"
...
log_pass "Partitioning the shared device and using it as a slog device" \
	" succeeded with zpool export/import operation"
