...
impl Locked {
    ...
    async fn flush_checkpoint(
        ...
        let merge_progress_phys = match (self.merge.as_ref(), new_index) {
            ...
            (Some(ms), Some(new_index_phys)) => Some(MergeProgressPhys {
                rebalance_log: ms.remap.as_ref().map(|remap| remap.log_phys.clone()),
    ...
    async fn resume_merge_task(
        ...
        let remap = match progress.rebalance_log {
            None => None,
            Some(log_phys) => Some(
                RemapState::open(
                    self.block_access.clone(),
                    self.slab_allocator.access(),
                    log_phys,
                )
                .await,
            ),
    ...
    async fn try_start_merge_task(
        ...
        let remap = match self.block_allocator.rebalance_init() {
            ...
            Some(map) => {
                ...
                Some(
                    RemapState::create(self.block_access.clone(), self.slab_allocator.clone(), map)
                        .await,
                )
            }
        };
    ...
    async fn rotate_index(&mut self, old_index: &mut IndexRun, new_index: IndexRun) {
            ...
            for (key, pc) in self.pending_changes.iter_mut() {
                match pc {
                    PendingChange::Insert(value) => {
                        ...
                        assert_eq!(remap.remap(extent).unwrap(), extent.location);
                    }
                    PendingChange::UpdateAtime(UpdateAtime(value, _)) => {
                        ...
                        match remap.remap(value.extent().unwrap()) {
                            Some(location) => {
        ...
        if let Some(mut remap) = merge.remap {
            // Free up the extents that have been allocated for the cache rebalancing
            remap.log_phys.clear(&self.slab_allocator);
        }
