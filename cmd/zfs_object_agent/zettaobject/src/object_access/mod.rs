...
impl From<Option<String>> for S3Credentials {
    fn from(credentials_profile: Option<String>) -> Self {
        match credentials_profile {
...
impl From<Option<String>> for BlobCredentials {
    fn from(credentials_profile: Option<String>) -> Self {
        match credentials_profile {
...
impl<'de> Deserialize<'de> for ObjectAccessProtocol {
    ...
    /// tagged with "protocol". Next, we try to do the wire protocol deserialization; this converts
    /// the credentials_profile into a platform-specific Credential. If this also fails to
    /// deserialize, it will fall back to the S3Legacy struct, which has the necessary
    /// information to create an S3 Protocol struct, but was written by something without
    /// protocol tag support.
        ...
        D: Deserializer<'de>,
    {
        ...
        match Upgrader::deserialize(d)? {
            Upgrader::Serialized(Serialized::S3 {
                endpoint,
                region,
                credentials,
            }) => Ok(Self::S3 {
